...
def get_datetime_from_old_file(user_username: str) -> str:
    ...
    if os.name == 'nt':
        res = date_time_obj.strftime('%Y-%m-%dT%H.%M')
    else:
        res = date_time_obj.strftime('%Y-%m-%dT%H:%M')
...
def rename_old_file(user_username: str) -> None:
    ...
    while True:
        try:
            ...
            if os.path.exists(new_file_path):  # На линуксе исключение не бросается автоматически
...
def todos_formatted(key: str, current_user_todos: dict) -> str:
    ...
    user_todos_formatted = map(lambda s: f"- {s[:46]}…" if len(s) > 46 else f"- {s}", user_todos)
